...
export default function Players() {
    ...
    function addPlayer(){
        ...
        setName('')
        localStorage.setItem('players', JSON.stringify([...players, {name:name,bets:1,chips:0}]))
    }
    useEffect(()=>{
        localStorage.getItem('players') && setPlayers(JSON.parse(localStorage.getItem('players')!))
    },[])
    function addBet(bet:number, player: string){
        setPlayers(players.map(p => p.name === player ? {...p, bets: bet} : p))
        localStorage.setItem('players', JSON.stringify(players.map(p => p.name === player ? {...p, bets: bet} : p)))
    }
    return (
        <section  id="players">
            <h2>Gracze</h2>
                ...
                <tbody>
        {
            players.map((player :any ,index:number) =>{
                return (<tr key={index}>
                    <td>{player.name}</td>
                    <td>{player.bets}</td>
                    <td><Plus onClick={() => addBet(player.bets + 1, player.name)}/></td>
                </tr>)
            })
        }
{/*
